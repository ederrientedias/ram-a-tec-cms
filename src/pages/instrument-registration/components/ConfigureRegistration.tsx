...
export const ConfigureRegistration = () => {
  ...
  return (
    <div className="grid grid-cols-12 gap-x-3 divide-x divide-gray-300">
      <div className={`flex flex-col gap-4 mt-12 ${!showPreview ? 'col-span-12' : 'col-span-7'}`}>
                ...
                return (
                  <Accordion key={item.id} type="single" collapsible className="w-full">
                    <AccordionItem value={item.id}>
                      ...
                      <AccordionContent>
                        <div className="w-full flex items-center justify-end gap-2 border-t py-2">
                          <Label
                            key={item.id}
                            className="h-9 flex items-center gap-3 hover:bg-accent/50 rounded-md border px-3 py-2 has-[[aria-checked=true]]:border-blue-600 has-[[aria-checked=true]]:bg-blue-50 cursor-pointer"
                          >
                            <Checkbox
                              id={item.id}
                        ...
                        <div className="grid grid-cols-3 gap-x-2 gap-y-3 mt-4 mb-4">
                          {item.fields.map((field: IField) => (
                            <Label
                              key={field.id}
                              className="flex items-center gap-2 hover:bg-accent/50 rounded-lg border p-3 has-[[aria-checked=true]]:border-blue-600 has-[[aria-checked=true]]:bg-blue-50 cursor-pointer"
                            >
                              <Checkbox
                                id={field.id}
      ...
      <div
        className={`
           h-full flex flex-col items-start col-span-5 pl-2 transition-all duration-500 transform
          ${
            showPreview
              ? 'translate-x-0 opacity-100 w-full'
              : 'translate-x-full opacity-0 w-0 hidden'
          }
        `}
      >
        <FormPreview data={forms} />
      </div>
    </div>
  );
};
