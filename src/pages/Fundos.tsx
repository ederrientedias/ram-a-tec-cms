...
const Fundos = () => {
  ...
  const filteredFundos = fundos.filter(
    (fundo) =>
      ...
      fundo.idFundo.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="space-y-6">
              ...
              {filteredFundos.length > 0 ? (
                filteredFundos.map((fundo) => (
                  <TableRow key={fundo.id}>
                    <TableCell className="font-medium">{fundo.idFundo}</TableCell>
                    <TableCell>{fundo.nome}</TableCell>
                    <TableCell>{fundo.produto}</TableCell>
                    <TableCell>{fundo.tipo}</TableCell>
                    <TableCell>{fundo.categoria}</TableCell>
                    <TableCell>{new Date(fundo.dataInicio).toLocaleDateString('pt-BR')}</TableCell>
                    <TableCell>R$ {fundo.valorCota.toFixed(2)}</TableCell>
                    <TableCell>{fundo.rentabilidadeMensal.toFixed(2)}%</TableCell>
                    <TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>
              {editingFundo ? "Editar Fundo" : "Cadastrar Novo Fundo"}
            </DialogTitle>
            <DialogDescription>
              Preencha as informações do fundo abaixo.
            </DialogDescription>
          </DialogHeader>
